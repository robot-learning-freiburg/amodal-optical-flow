...
try:
    ...
    nattenav_cuda = load(
        "nattenav_cuda",
        ["core/cuda/nattenav_cuda.cpp", "core/cuda/nattenav_cuda_kernel.cu"],
        verbose=True,
    )
    print("[Finished 1/2]")
    nattenqkrpb_cuda = load(
        "nattenqkrpb_cuda",
        ["core/cuda/nattenqkrpb_cuda.cpp", "core/cuda/nattenqkrpb_cuda_kernel.cu"],
        verbose=False,
    )
    print("[Finished 2/2]")
except Exception:
...
class NATTENAVFunction(Function):
    ...
    def forward(ctx, attn, value):
        ...
        out = nattenav_cuda.forward(attn, value)[0]
    ...
    def backward(ctx, grad_out):
        outputs = nattenav_cuda.backward(grad_out.contiguous(), *ctx.saved_variables)
...
class NATTENQKRPBFunction(Function):
    ...
    def forward(ctx, query, key, rpb):
        ...
        key = key.contiguous()
        attn = nattenqkrpb_cuda.forward(query, key, rpb.contiguous())[0]
    ...
    @custom_bwd
    def backward(ctx, grad_out):
        outputs = nattenqkrpb_cuda.backward(grad_out.contiguous(), *ctx.saved_variables)
...
class selfattentionlayer_nat(nn.Module):
    ...
    def forward(self, x, size, context):
        ...
        q = (
            (self.q(qk) * self.scale)
            .reshape(B, H, W, self.num_heads, 16)
            .permute(0, 3, 1, 2, 4)
        )
