...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, tgt_token, size=None, ids_keep=None):
        ...
        if ids_keep is not None:
            tgt_token = torch.gather(
                tgt_token,
                dim=1,
                index=ids_keep.unsqueeze(-1).repeat(1, 1, tgt_token.shape[-1]),
            )
        ...
        for idx, corners in enumerate(indices):
            ...
            res.append(
                self.multi_head_attn(
                    q[:, 2 + idx : 3 + idx, :],
                    k[:, h0:h1, w0:w1, :].reshape(B, -1, C),
                    v[:, h0:h1, w0:w1, :].reshape(B, -1, C),
                )
            )
...
class CrossAttentionLayer_convk3s2(nn.Module):
    def __init__(
        ...
        self.q_coarse, self.k_coarse, self.v_coarse = (
            nn.Linear(query_token_dim, qk_dim, bias=True),
            nn.Linear(tgt_token_dim, qk_dim, bias=True),
            nn.Linear(tgt_token_dim, v_dim, bias=True),
        )
    ...
    def forward(self, query, tgt_token, size):
        ...
        q, k, v = (
            self.q_coarse(query),
            self.k_coarse(tgt_token_coarse),
            self.v_coarse(tgt_token_coarse),
        )
        res_coarse = []
