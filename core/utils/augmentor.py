...
class AmodalFlowAugmentor:
    ...
    def resize_amodal_flow_map(self, flow, valid, fx=1.0, fy=1.0):
        if fx > 1.0 or fy > 1.0:
            interpolation = cv2.INTER_NEAREST
        else:
            interpolation = cv2.INTER_LINEAR

        flow = cv2.resize(flow, None, fx=fx, fy=fy, interpolation=interpolation)
        flow = flow * [fx, fy]

        if not np.all(valid > 0):
            valid = cv2.resize(valid, None, fx=fx, fy=fy, interpolation=interpolation)
    ...
    def spatial_transform(self, img1, img2, flow, valid, sseg):
        pad_t = 0
        pad_b = 0
        pad_l = 0
        pad_r = 0
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            flow, valid = zip(
                *[
                    self.resize_amodal_flow_map(f, v, fx=scale_x, fy=scale_y)
                    for f, v in zip(flow, valid)
                ]
            )
            sseg = [
                cv2.resize(
                    s, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_NEAREST
                )
                for s in sseg
            ]
        ...
        if self.do_flip:
            if np.random.rand() < 0.5:  # h-flip
                img1 = img1[:, ::-1]
                img2 = img2[:, ::-1]
                flow = [f[:, ::-1] * [-1.0, 1.0] for f in flow]
                valid = [v[:, ::-1] for v in valid]
                sseg = [s[:, ::-1] for s in sseg]
        ...
        img2 = img2[y0 : y0 + self.crop_size[0], x0 : x0 + self.crop_size[1]]
        flow = [
            f[y0 : y0 + self.crop_size[0], x0 : x0 + self.crop_size[1]] for f in flow
        ]
        valid = [
            v[y0 : y0 + self.crop_size[0], x0 : x0 + self.crop_size[1]] for v in valid
        ]
        sseg = [
            s[y0 : y0 + self.crop_size[0], x0 : x0 + self.crop_size[1]] for s in sseg
        ]

        return img1, img2, flow, valid, sseg

    def __call__(self, img1, img2, flow, valid, sseg):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        img1, img2, flow, valid, sseg = self.spatial_transform(img1, img2, flow, valid, sseg)

        img1 = np.ascontiguousarray(img1)
        img2 = np.ascontiguousarray(img2)
        flow = [np.ascontiguousarray(f) for f in flow]
        valid = [np.ascontiguousarray(v) for v in valid]
        sseg = [np.ascontiguousarray(s) for s in sseg]
