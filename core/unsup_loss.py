...
def compute_range_map(flow):
    ...
    idx_batch_offset = (
        batch_range.repeat(1, input_height, input_width) * input_height * input_width
    )
    ...
    for di in range(2):
        for dj in range(2):
            ...
            weights_i = (1.0 - di) - (-1) ** di * valid_offsets[:, 0]
            weights_j = (1.0 - dj) - (-1) ** dj * valid_offsets[:, 1]
...
def cal_unsup_loss(image1, image2, flow_forward, flow_backward, gamma):
    ...
    for i in range(n_predictions):
        with torch.no_grad():
            occlusion_mask = compute_occlusion(
                flow_ij=flow_forward[i],
                flow_ji=flow_backward[-1],
