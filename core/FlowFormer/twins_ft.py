...
def get_2d_sincos_pos_embed_from_grid(embed_dim, grid, cossin=False):
    ...
    emb_h = get_1d_sincos_pos_embed_from_grid(
        embed_dim // 2, grid[0], cossin=cossin
    )  # (H*W, D/2)
    emb_w = get_1d_sincos_pos_embed_from_grid(
        embed_dim // 2, grid[1], cossin=cossin
    )  # (H*W, D/2)
...
def interpolate_pos_embed(model, checkpoint_model, remove_index=True):
    ...
    relative_position_bias_table_keys = [
        k for k in checkpoint_model.keys() if "relative_position_bias_table" in k
    ]
    for table_key in relative_position_bias_table_keys:
        ...
        if nH1 != nH2:
            ...
            if L1 != L2:
                ...
                checkpoint_model[table_key] = table_pretrained_resized.view(
                    nH2, L2
                ).permute(1, 0)
...
class Twins(nn.Module):
    ...
    def __init__(
        ...
        for idx, embed_dim in enumerate(embed_dims):
            ...
            pe = nn.Parameter(
                torch.zeros(1, num_patches[idx], embed_dim), requires_grad=True
            )
            setattr(self, f"pos_block_{idx}", pe)
            _, N, C = pe.shape
            pos_embed = get_2d_sincos_pos_embed(C, int(N**0.5), cls_token=False)
            getattr(self, f"pos_block_{idx}").data.copy_(
